...
export function TaskCard({ task }: Props) {
  ...
  return (
    <div className={`card-base text-left w-full ${priorityBorderClass[task.priority]}`}>
      ...
      {/* Task title */}
      <p className="text-xs font-medium text-ink leading-tight mb-2 line-clamp-3">
        {task.title}
      </p>
      ...
      {task.notes && (
        <div className="flex items-start gap-1 mb-2">
          <StickyNote size={10} className="text-ink-ghost mt-0.5 flex-shrink-0" />
          <p className="text-[10px] text-ink-muted leading-tight line-clamp-2">{task.notes}</p>
        </div>
      )}
      ...
      >
        ...
        <div className="flex items-center gap-2 min-w-0">
          {task.assignee && (
            <div className="flex items-center gap-1 text-[10px] text-ink-muted min-w-0">
          ...
          {dueDateStr && (
            <div
              className={`flex items-center gap-1 text-[10px] flex-shrink-0 ${
                isOverdue ? 'text-red-600 dark:text-red-400 font-semibold' : 'text-ink-muted'
              }`}
            >
              <Calendar size={10} />
